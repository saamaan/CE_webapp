...
hook before_template => sub {
    ...
    $tokens->{info_email_link} = '<a href="mailto:info@smellbetter.org">info@smellbetter.org</a>';
    $tokens->{host} = host();
...
EOT
            ...
            unless ($m->active or $tokens->{message}) {
                $tokens->{warning} ||= <<'EOT';
Your account is cancelled. <a href="mailto:info@smellbetter.org">Email us</a> if this is incorrect.
EOT
            }
        }
    }
...
sub update_payment_profile_message {
	...
	#SP
	#This cause the website to blowup when the user logged in (observed it for a new member) and immediately clicked on "view profile"
	when (undef) { return ""; }
...
<p>If you have any feedback for our co-op, please send it to us at <a href="mailto:info@smellbetter.org">info@smellbetter.org</a>.</p>
EOT
        forward '/member/view';
    }
    template 'cancel', {
